...
import SofaPic from "../../Pictures InstaPage/pic10.jpg";
...
const Sofas = () => {
  ...
  const sofas = [...Array(12)].map((_, index) => ({
    id: index + 1,
    name: `Sofa name ${index + 1}`,
    price: Math.floor(Math.random() * 20000) + 1000,
  }));

  // Calculate the highest price among lamps
  const highestPrice = Math.max(...sofas.map((sofa) => sofa.price));

  const indexOfLastSofa = currentPage * itemsPerPage;
  const indexOfFirstSofa = indexOfLastSofa - itemsPerPage;
  const currentLamps = sofas.slice(indexOfFirstSofa, indexOfLastSofa);

  const totalPages = Math.ceil(sofas.length / itemsPerPage);
  ...
  return (
    <>
      ...
      <h1 className="fontStyling mt-4">Sofas</h1>
 ...
 <option value="best-selling" selected>
      ...
      <div className="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4 mt-1">
        {currentLamps.length > 0 ? (
          currentLamps.map((sofa) => (
            <div className="col" key={sofa.id}>
              <div className="card h-100 shadow-sm product-card border-1">
                <Link
                  className="text-dark a_"
                  to={`/accessories/lamps/${sofa.id}`}
                >
                  <img
                    src={SofaPic}
                  ...
                  <div className="card-body py-4 mt-3 text-start">
                    <h5 className="card-title a_title">{sofa.name}</h5>
                    <p className="card-text a_price">Rs.{sofa.price}.00 PKR</p>
                  </div>
                </Link>
              </div>
            </div>
          ...
          <div className="col-12 text-center mt-4">
            <p className="text-muted">No Sofas found matching your criteria.</p>
          </div>
        )}
      </div>
