...
import TablePic from "../../Pictures InstaPage/pic9.jpg";
...
const Table = () => {
  ...
  const tables = [...Array(12)].map((_, index) => ({
    id: index + 1,
  ...
  const highestPrice = Math.max(...tables.map((table) => table.price));

  const indexOfLastTable = currentPage * itemsPerPage;
  const indexOfFirstTable = indexOfLastTable - itemsPerPage;
  const currentLamps = tables.slice(indexOfFirstTable, indexOfLastTable);
  ...
  return (
    <>
      ...
      <h1 className="fontStyling mt-4">Tables</h1>
              ...
              <div className="card h-100 shadow-sm product-card border-1">
                ...
                >
                  ...
                  <div className="card-body py-4 mt-3 text-start">
                    <h5 className="card-title a_title">{table.name}</h5>
                    <p className="card-text a_price">Rs.{table.price}.00 PKR</p>
                  </div>
                </Link>
              </div>
            </div>
