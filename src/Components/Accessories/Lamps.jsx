...
const Lamps = () => {
  ...
  const toggleFilterCard = () => {
    ...
    setFilterPosition({ top: buttonRect.bottom + window.scrollY, left: buttonRect.left });
    setShowFilter((prev) => !prev);
    setShowPrice(false);
  };
  ...
  const togglePrice = () => {
    ...
    setPricePosition({ top: buttonRect.bottom + window.scrollY, left: buttonRect.left });
    setShowPrice((prev) => !prev);
    setShowFilter(false);
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (filterRef.current && !filterRef.current.contains(event.target) && !filterButtonRef.current.contains(event.target)) {
        setShowFilter(false);
      }
      if (priceRef.current && !priceRef.current.contains(event.target) && !priceButtonRef.current.contains(event.target)) {
        setShowPrice(false);
      }
    };
    document.addEventListener("mousedown", handleClickOutside);
  ...
  return (
    <div className="container">
      ...
      {showFilter && (
        <div ref={filterRef} className="card mt-3 border-black" style={{ position: "absolute", top: filterPosition.top, left: filterPosition.left, width: "300px", zIndex: 1000 }}>
          <div className="card-body p-3">
            <div className="d-flex justify-content-between align-items-center">
      ...
      {showPrice && (
        <div ref={priceRef} className="card mt-3 border-black" style={{ position: "absolute", top: pricePosition.top, left: pricePosition.left, width: "315px", zIndex: 1000 }}>
          <div className="card-body p-3">
            <div className="d-flex justify-content-between align-items-center">
              <p className="card-title m-0 text-muted me-5">
